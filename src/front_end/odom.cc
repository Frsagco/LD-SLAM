...
/* ---------------------------------- ROS 2 ----------------------------------------- */
dlo::OdomNode::OdomNode() : Node("dlo_node")
{
...
/* this->icp_sub = this->nh.subscribe("pointcloud", 1, &dlo::OdomNode::icpCB, this);
  ...
  this->icp_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>("pointcloud", 1, std::bind(&dlo::OdomNode::icpCB, this));
  this->imu_sub = this->create_subscription<sensor_msgs::msg::Imu>("imu", 1, std::bind(&dlo::OdomNode::imuCB, this));
  ...
  this->kf_pub = this->create_publisher<nav_msgs::msg::Odometry>("kfs", 1, true);
  this->keyframe_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("keyframe", 1, true);
...
{
  // Version
  this->version_ = std::string("0.0.0");
  this->version_ = this->declare_parameter("~dlo/version", this->version_);

  // Frames
  this->odom_frame = std::string("odom");
  this->odom_frame = this->declare_parameter("~dlo/odomNode/odom_frame", this->odom_frame);

  this->child_frame = std::string("base_link");
  this->child_frame = this->declare_parameter("~dlo/odomNode/child_frame", this->odom_frame);
  ...
  this->gravity_align_ = this->declare_parameter("~dlo/gravityAlign", this->gravity_align_);

  // Keyframe Threshold
  this->keyframe_thresh_dist_ = 0.1;
  this->keyframe_thresh_rot_ = 1.0;
  this->keyframe_thresh_dist_ = this->declare_parameter("~dlo/odomNode/keyframe/threshD", this->keyframe_thresh_dist_);
  this->keyframe_thresh_rot_ = this->declare_parameter("~dlo/odomNode/keyframe/threshR", this->keyframe_thresh_rot_);

  // Submap
  this->submap_knn_ = 10;
  this->submap_kcv_ = 10;
  this->submap_kcc_ = 10;
  this->submap_knn_ = this->declare_parameter("~dlo/odomNode/submap/keyframe/knn", this->submap_knn_);
  this->submap_kcv_ = this->declare_parameter("~dlo/odomNode/submap/keyframe/kcv", this->submap_kcv_);
  this->submap_kcc_ = this->declare_parameter("~dlo/odomNode/submap/keyframe/kcc", this->submap_kcc_);

  // Initial Position
  this->initial_pose_use_ = false;
  this->initial_pose_use_ = this->declare_parameter("~dlo/odomNode/initialPose/use", this->initial_pose_use_);
  ...
  qy = 0.0;
  qz = 0.0;
  px = this->declare_parameter("~dlo/odomNode/initialPose/position/x", px);
  py = this->declare_parameter("~dlo/odomNode/initialPose/position/y", py);
  pz = this->declare_parameter("~dlo/odomNode/initialPose/position/z", pz);
  qw = this->declare_parameter("~dlo/odomNode/initialPose/orientation/w", qw);
  qx = this->declare_parameter("~dlo/odomNode/initialPose/orientation/x", qx);
  qy = this->declare_parameter("~dlo/odomNode/initialPose/orientation/y", qy);
  qz = this->declare_parameter("~dlo/odomNode/initialPose/orientation/z", qz);
  ...
  this->crop_size_ = 1.0;
  this->crop_use_ = this->declare_parameter("~dlo/odomNode/preprocessing/cropBoxFilter/use", this->crop_use_);
  this->crop_size_ = this->declare_parameter("~dlo/odomNode/preprocessing/cropBoxFilter/size", this->crop_size_);
  ...
  this->vf_scan_use_ = this->declare_parameter("~dlo/odomNode/preprocessing/voxelFilter/scan/use", this->vf_scan_use_);
  this->vf_scan_res_ = this->declare_parameter("~dlo/odomNode/preprocessing/voxelFilter/scan/res", this->vf_scan_res_);
  this->vf_submap_use_ = this->declare_parameter("~dlo/odomNode/preprocessing/voxelFilter/submap/use", this->vf_submap_use_);
  this->vf_submap_res_ = this->declare_parameter("~dlo/odomNode/preprocessing/voxelFilter/submap/res", this->vf_submap_res_);

  // Adaptive Parameters
  this->adaptive_params_use_ = false;
  this->adaptive_params_use_ = this->declare_parameter("~dlo/adaptiveParams", this->adaptive_params_use_);
  ...
  this->imu_use_ = this->declare_parameter("~dlo/imu", this->imu_use_);
  this->imu_calib_time_ = this->declare_parameter("~dlo/odomNode/imu/calibTime", this->imu_calib_time_);
  this->imu_buffer_size_ = this->declare_parameter("~dlo/odomNode/imu/bufferSize", this->imu_buffer_size_);
  ...
  this->gicp_min_num_points_ = this->declare_parameter("~dlo/odomNode/gicp/minNumPoints", this->gicp_min_num_points_);
  this->gicps2s_k_correspondences_ = this->declare_parameter("~dlo/odomNode/gicp/s2s/kCorrespondences", this->gicps2s_k_correspondences_);
  this->gicps2s_max_corr_dist_ = this->declare_parameter("~dlo/odomNode/gicp/s2s/maxCorrespondenceDistance", this->gicps2s_max_corr_dist_);
  this->gicps2s_max_iter_ = this->declare_parameter("~dlo/odomNode/gicp/s2s/maxIterations", this->gicps2s_max_iter_);
  this->gicps2s_transformation_ep_= this->declare_parameter("~dlo/odomNode/gicp/s2s/transformationEpsilon", this->gicps2s_transformation_ep_);
  this->gicps2s_euclidean_fitness_ep_= this->declare_parameter("~dlo/odomNode/gicp/s2s/euclideanFitnessEpsilon", this->gicps2s_euclidean_fitness_ep_);
  this->gicps2s_ransac_iter_=this->declare_parameter("~dlo/odomNode/gicp/s2s/ransac/iterations", this->gicps2s_ransac_iter_);
  this->gicps2s_ransac_inlier_thresh_=this->declare_parameter("~dlo/odomNode/gicp/s2s/ransac/outlierRejectionThresh", this->gicps2s_ransac_inlier_thresh_);
  this->gicps2m_k_correspondences_= this->declare_parameter("~dlo/odomNode/gicp/s2m/kCorrespondences", this->gicps2m_k_correspondences_);
  this->gicps2m_max_corr_dist_= this->declare_parameter("~dlo/odomNode/gicp/s2m/maxCorrespondenceDistance", this->gicps2m_max_corr_dist_);
  this->gicps2m_max_iter_= this->declare_parameter("~dlo/odomNode/gicp/s2m/maxIterations", this->gicps2m_max_iter_);
  this->gicps2m_transformation_ep_=this->declare_parameter("~dlo/odomNode/gicp/s2m/transformationEpsilon", this->gicps2m_transformation_ep_);
  this->gicps2m_euclidean_fitness_ep_= this->declare_parameter("~dlo/odomNode/gicp/s2m/euclideanFitnessEpsilon", this->gicps2m_euclidean_fitness_ep_);
  this->gicps2m_ransac_iter_ = this->declare_parameter("~dlo/odomNode/gicp/s2m/ransac/iterations", this->gicps2m_ransac_iter_);
  this->gicps2m_ransac_inlier_thresh_=this->declare_parameter("~dlo/odomNode/gicp/s2m/ransac/outlierRejectionThresh", this->gicps2m_ransac_inlier_thresh_);
}
